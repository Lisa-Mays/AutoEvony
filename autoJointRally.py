...
class AutoJointRally:
    def __init__(self,exit_flag,settingJoinRally,test):
        self.settingJoinRally = settingJoinRally
        self.exit_flag = exit_flag
        
        self.test = test
        self.ip = ".".join(map(str, [(self.test[0] >> 24) & 255, (self.test[0] >> 16) & 255, (self.test[0] >> 8) & 255, self.test[0] & 255]))
        self.port = self.test[1]
    ...
    def bosNFilter(self):
        while 1:
            ...
            click = self.sc.find_img(self.imgerror3,screenshot, 0.9, 1)
            if click != False:
                self.sc.esc()
                tm.sleep(2)
                self.checkAlliance = 1
            ...
            if click != False:
                ...
                click = self.sc.find_img(self.imgPVP,screenshot, 0.9, 1)
                if click != False:
                    self.sc.swipe(280,650,280,350,1500)
    ...
    def chosechamp(self):
        while 1:
            ...
            if  self.checkChamp == 1:
                click = self.sc.find_img(self.imgselectChamp,screenshot, 0.99, 1)
                if click != False:
                    self.sc.click(click[0][0], click[0][1])
                    self.checkChamp = 0
                    self.checkInWar = 1
